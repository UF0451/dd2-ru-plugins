...
local marker_types =
{
  ...
  {
    label = "Сундуки (ОБ)",
    tooltip = "Также известны как «затонувшие сундуки». Большинство из них доступны только после прохождения игры.",
...
local marker_label = "Откройте карту!"
local update_marker_count = function(game_count, script_count, remaining_count, total_count)
  marker_label = ("Значки: %d игры, %d альманаха, %d свободно (максимум %d)"):format(game_count, script_count, remaining_count, total_count)
end
...
local draw_marker_type_settings = function(setting_prefix, marker_type, marker_settings)
  ...
  local dirty = nil
  local changed, new_value = imgui.drag_int("Тип иконки", marker_settings[icon_type_key], 1, 0, 75)
  if changed then
    marker_settings[icon_type_key] = new_value
    dirty = true
  end
  if imgui.tree_node("Цвет иконки") then
...
re.on_draw_ui(function()
  ...
  if imgui.tree_node("Альманах Восставшего") then
    if marker_label ~= nil then
      imgui.text(marker_label)
    end
    if show_exception then
      imgui.text_colored(
        "Произошла ошибка при получении статуса предметов. Пожалуйста, сообщите об этом gibbedу.",
        0xFF0000FF)
    end
    if show_warning then
      imgui.text_colored(
        "Достигнут лимит иконок! Иконки ограничены."
    ...
    for key_index, marker_type_name in ipairs(settings.marker_order) do
      ...
      if imgui.is_item_hovered() then
        ...
        if marker_settings.unacquired_show then
          imgui.set_tooltip("Показать неполученные " .. marker_type.label)
        else
          imgui.set_tooltip("Скрыть неполученные " .. marker_type.label)
        end
        imgui.end_tooltip()
      end
      if changed then
      ...
      if imgui.is_item_hovered() then
        ...
        if marker_settings.acquired_show then
          imgui.set_tooltip("Показать полученные " .. marker_type.label)
        else
          imgui.set_tooltip("Скрыть полученные " .. marker_type.label)
        end
        imgui.end_tooltip()
      end
      if changed then
      ...
      if type(marker_type.data) ~= "table" or type(marker_type.data.locations) ~= "table" then
        ...
        tree_node = imgui.tree_node(marker_type.label .. " (файл данных отсутствует!)")
        imgui.pop_style_color()
      else
      ...
      if tree_node then
        if imgui.tree_node("Настройки неполученных") then
        ...
        if imgui.tree_node("Настройки полученных") then
