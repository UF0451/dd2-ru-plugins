...
local function render_keybind(keybind, show_keys)
    ...
    local shift_changed, should_disable = imgui.checkbox("Отключить 'Смена навыка оружия'?", not with_shift) -- Notice the negation here, UI feels more intuitive when patching is "V"
...
re.on_draw_ui(function()
    if imgui.tree_node("Пользовательские клавиши") then

        if imgui.tree_node("Расширенные настройки") then
            imgui.spacing()
            imgui.spacing()
            imgui.spacing()
            imgui.begin_rect()
            imgui.text("Назначьте клавиши, которые нельзя сменить в игре")
            imgui.spacing()
            imgui.spacing()
        ...
        imgui.text("Принудительно отключить клавиши 'Ctrl+X':")
        imgui.spacing()
