...
--settings
local _config={
    {name="newlinewidth",type="int",default=20,min=1,max=250,label="Ширина строки"},
    {name="newlinewidthaffectoriginaltext",type="bool",default=true,label="Ширина строки влияет на оригинальный текст"},
    {name="ignoreArmorAndWeapon",type="bool",default=false,label="Игнорировать статы брони и оружия"},
    {name="ignoreArmorAndWeaponSpecialEffect",type="bool",default=false,label="Игнорировать спец. эффекты брони и оружия"},
    {name="removeOriginalText",type="bool",default=true,label="Удалять оригинальный текст описания"},
    {name="specifyTransFile",type="string",default="",label="Принудительный язык (нужен сброс скрипта)"},
    {name="enabled",type="bool",default=true,label="Включить мод (показывать доп. описания)"},
}
...
local FieldFormat={
    ...
    _WeaponName=nil,
    _WeaponId=nil,
    _PhysicalAttack="%s АТК",
    _SlashRate=nil,
    _StrikeRate=nil,
    _StrikeStore="%s Накопление оглушения",
    _MagicAttack="%s МагАТК",
    _Element=nil,
    _ElementStore="%s%% Зачарование стихией",
    _Shake=nil,
    _Blow="%s Сбивание",
    _StaminaReduce=nil,
    _ShakeGuard=nil,
    _BlowGuard="%s Защита от сбивания",
    _PoisonStore="%s Накопление яда",
    _SleepStore="%s Накопление сна",
    _SilentStore="%s Накопление молчания",
    _StoneStore="%s Накопление окаменения",
    _WaterStore="%s Накопление воды",
    _OilStore="%s Накопление масла",

    --app.ItemArmorParam
    _StyleNo=nil,
    _PhysicalDefence="%s Физ.Защ",
    _SlashDefenceRate="%s%% Сниж. от рубящего",
    _StrikeDefenceRate="%s%% Сниж. от дробящего",
    _MagicDefence="%s МагЗащ",
    _FireDefence="%s%% Сниж. урона огнём",
    _FireResist="%s%% Сопр. деб. огня",
    _IceDefence="%s%% Сниж. урона льдом",
    _IceResist="%s%% Сопр. деб. льда",
    _ThunderDefence="%s%% Сниж. урона молнией",
    _ThunderResist="%s%% Сопр. деб. молнии",
    _LightDefence="%s%% Сниж. урона светом",
    _DarkDefence="%s%% Сопр. деб. тьмы",
    _ShakeResistRate="%s%% Сопр. тряске",
    _BlowResistRate="%s%% Сопр. сбиванию",
    _PoisonResist="%s%% Сопр. яду",
    _SleepResist="%s%% Сопр. сну",
    _SilentResist="%s%% Сопр. молчанию",
    _StoneResist="%s%% Сопр. окаменению",
    _WaterResist="%s%% Сопр. воде",
    _OilResist="%s%% Сопр. маслу",
    --_Special=nil,
    --_SpecialValue=nil,
    --_SpecialValue2=nil,
    --_SpecialValue3=nil,
...
local ItemBuffFormat={
    ["AttackFactor"]="+%s%% Атака ",
    ["DefenceFactor"]="+%s%% Защита ",
    ["StatusConditionResistFactor"]= "+%s%% Сопр. статусам ",
    ["StaminaRecoverFactor"]= "%s%% Восст. выносливости ",
    ["Sec"]= "на %s секунд",
    ["MaxHpDamageFactor"]="-%s%% Потеря макс. HP ",
    ["Camp"]="Лагерь:"
}
        ...
        if tmpJobWindow~=nil then
            ...
            if tmpJobWindow._TxtAbilityInfoName:get_MessageId():ToString()~="00000000-0000-0000-0000-000000000000" then
                ...
                if abilityId~=nil and abilityId>0 then
                    if config.enabled then tmpJobWindow._TxtAbilityInfo:set_Message(GetOrAddSkillDesc(tmpJobWindow._TxtAbilityInfo:get_Message(),abilityId)) end
                end
            end
            tmpJobWindow=nil
        end
    ...
    function ()
        if tmpJobWindow~=nil then
            ...
            if cursor.ContenstsType==MainContentsInfoKindAbility then
                abilityId=cursor.Ability.AbilityID
                if abilityId~=nil and abilityId>0 then
                    if config.enabled then tmpJobWindow._TxtJobAbilityInfo:set_Message(GetOrAddSkillDesc(tmpJobWindow._TxtJobAbilityInfo:get_Message(),abilityId)) end
                end
            end
            tmpJobWindow=nil
        end
    ...
    if tmpStatusWindow~=nil then
        ...
        if tmpSkillInfo.DispType==3 then
            ...
            if SkillName2Id[nameText]~=nil then
                if config.enabled then tmpStatusWindow.Movie.TxtDetail:set_Message(GetOrAddSkillDesc(originalMessage,SkillName2Id[nameText])) end
            end
        end
        tmpStatusWindow=nil
        tmpSkillInfo=nil
