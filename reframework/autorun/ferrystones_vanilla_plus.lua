...
re.on_draw_ui(function()
	...
	if imgui.tree_node("Камни возврата Vanilla Plus") then
		local changed = false

		imgui.new_line()

		reset_config = imgui.button("Сбросить всё")
		...
		changed, config.free_portcrystals_enabled = imgui.checkbox("Бесплатная телепортация к порткристаллам",
		...
		-- ferrystone_drop_rates
		imgui.text("С врагов")
		changed, config.ferrystone_drop_rates["DeadEnemy"].enabled = imgui.checkbox("Включено",
			config.ferrystone_drop_rates["DeadEnemy"].enabled)
		config_changed = config_changed or changed

		imgui.begin_disabled(not config.ferrystone_drop_rates["DeadEnemy"].enabled)
		changed, config.ferrystone_drop_rates["DeadEnemy"].chance = imgui.slider_float("Шанс",
		...
		imgui.text("Сбор")
		changed, config.ferrystone_drop_rates["Gather"].enabled = imgui.checkbox("Включено",
			config.ferrystone_drop_rates["Gather"].enabled)
		config_changed = config_changed or changed

		imgui.begin_disabled(not config.ferrystone_drop_rates["Gather"].enabled)
		changed, config.ferrystone_drop_rates["Gather"].chance = imgui.slider_float("Шанс",
		...
		imgui.text("Из сундуков")
		changed, config.ferrystone_drop_rates["TreasureBox"].enabled = imgui.checkbox("Включено",
		...
		changed, config.ferrystone_drop_rates["TreasureBox"].chance = imgui.slider_float("Шанс",
		...
		imgui.text("Награды NPC")
		changed, config.ferrystone_drop_rates["Talk"].enabled = imgui.checkbox("Включено",
			config.ferrystone_drop_rates["Talk"].enabled)
		config_changed = config_changed or changed

		imgui.begin_disabled(not config.ferrystone_drop_rates["Talk"].enabled)
		changed, config.ferrystone_drop_rates["Talk"].chance = imgui.slider_float("Шанс",
