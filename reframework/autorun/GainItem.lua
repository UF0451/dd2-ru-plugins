...
local _config={
    {name="item", type="item", default=1}, -- Ключ 'item' обязателен для API
    {name="count", type="int", default=1, min=1, max=99}, -- Ключ 'count' обязателен
    {name="Действия", type="mutualbox"},
    {name="Отправить игроку", type="button", onClick=function() onClickFunc("Player") end, sameline=true},
    {name="Отправить на склад", type="button", onClick=function() onClickFunc("Storage") end},
    {name="Пешка 1", type="button", onClick=function() onClickFunc("Pawn",0) end, sameline=true},
    {name="Пешка 2", type="button", onClick=function() onClickFunc("Pawn",1) end, sameline=true},
    {name="Пешка 3", type="button", onClick=function() onClickFunc("Pawn",2) end},
    {name="Удалить у игрока", type="button", onClick=function() onClickFunc("Player",nil,true) end},
    {name="Избранное", type="mutualbox"},
    {name="Добавить", type="button", onClick=function() onAddToFavList() end, sameline=true},
    {name="Удалить", type="button", onClick=function() onRemoveFromFavList() end},
    {name="Список", type="buttonN", onClick=function(...) onFavListClick(...) end, default={}},
    {type="mutualboxend"},
    {type="author"}
}
...
onAddToFavList=function()
    ...
    if item~=nil then
        ...
        table.insert(list,favitem)
        config["Список"] = list
    end
end

onRemoveFromFavList=function()
    local list = config["Список"]
    if config.item==nil or list==nil then return end
