...
local _pasteCounter = 0
local _pendingFix = {}
local function _CyrillicInputText(label, text)
    local suffix = ""
    if _pendingFix[label] then
        text = _pendingFix[label]
        _pasteCounter = _pasteCounter + 1
        suffix = "##cyr" .. tostring(_pasteCounter)
        _pendingFix[label] = nil
    end
    ...
    if corrected ~= newText then
        imgui.text_colored(corrected, 0xFF4FFAA1)
        imgui.same_line()
        if imgui.small_button("Вставить##" .. label) then
            _pendingFix[label] = corrected
        end
...
local function DrawIt(modname,configfile,_config,config,OnChange,dontInitHotkey,font)
    ...
    re.on_draw_ui(function()
        ...
        if not isFontChanged and _G.ru_font then
            imgui.push_font(_G.ru_font)
            isFontChanged=true
        end
	    ...
	    if imgui.tree_node(modname) then
            ...
            for _,para in ipairs (_config) do
                ...
                elseif para.type=="font" or para.type=="string" then
        		    changed , config[key]= _CyrillicInputText(label .. title_postfix, config[key] or para.default)
                    _changed=changed or _changed
                ...
                elseif para.type=="item" then
                    if para.enableSearch==true or para.enableSearch==nil then
                        ...
                        changed, para.tmp_SearchText = _CyrillicInputText("<-searh "..key, para.tmp_SearchText)
                        imgui.pop_item_width()
                        imgui.same_line()
